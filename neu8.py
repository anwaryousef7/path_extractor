...
class UltimateLogAnalyzer(ctk.CTk):
    def __init__(self):
        ...
        self.title("LogMaster Pro - Ultimate Multi-Format Edition")
        ...
        # Sets to collect unique entities for the new filter
    ...
    def setup_ui(self):
        ...
        self.search_entry = ctk.CTkEntry(self.filter_frame, placeholder_text="Live Search: Text, Details...", textvariable=self.search_var, width=250, height=35)
        self.search_entry.pack(side="left", padx=10, pady=15)

        # 2. Entity Dropdown (IP, Email, Domain) - NEW FEATURE
        self.entity_var = ctk.StringVar(value="All Entities")
        self.entity_combo = ctk.CTkComboBox(self.filter_frame, variable=self.entity_var, values=["All Entities"], width=250, height=35, command=self.apply_filters)
                ...
                with open(filepath, 'r', encoding='utf-8', errors='ignore') as file:
                    for line in file:
                        try:
                            ...
                            # Format 1: HAProxy / SCCM (Updated for IPv6 + Ports)
                            if "<![LOG[" in line:
                                msg_match = re.search(r'<!\[LOG\[(.*?)\] LOG\]!', line)
                                msg = msg_match.group(1).strip() if msg_match else ""
                                
                                # Dates handling German/special quotes
                                ...
                                # Extract Component/Protocol for richer details
                                comp_match = re.search(r'component=["\'„“]([^"\'„“]+)["\'„“]', line)
                                component_info = f" [Comp: {comp_match.group(1)}]" if comp_match else ""
                                details = msg + component_info
    ...
    def apply_filters(self, *args):
        ...
        for row in self.parsed_data:
            ...
            # 3. Entity (IP/Email/Domain) Filter
    ...
    def resolve_and_locate(self, source, details):
        ...
        try:
            # Cleanup source (remove port if attached to IPv6/IPv4 for geolocation)
            clean_source = source.split(']')[0].replace('[', '') if '[' in source else source
            if clean_source.count(':') > 1 and not clean_source.endswith(':'): # Likely IPv6 with port
                 # Rough extraction of base IPv6
