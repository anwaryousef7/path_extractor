...
class UltimateLogAnalyzer(ctk.CTk):
    def __init__(self):
        ...
        self.title("LogMaster Pro - AI & Extraction Edition")
        ...
        # Sets for general UI and targeted extraction
    ...
    def setup_ui(self):
        ...
        # NEW FEATURE BUTTON: Entity Extractor
        self.extract_btn = ctk.CTkButton(self.sidebar_frame, text="2. Entity Extractor (IP/Email)", command=self.open_entity_extractor, height=45, fg_color="#D35400", hover_color="#A04000")
        ...
        ctk.CTkLabel(self.details_frame, text="Contextual Log Details", font=ctk.CTkFont(weight="bold")).pack(pady=5)
    ...
    # --- NEW FEATURE: ENTITY EXTRACTOR WINDOW ---
    def open_entity_extractor(self):
        ...
        ext_win.title("Entity Extractor & Exporter")
        ext_win.geometry("500x600")
        ext_win.attributes('-topmost', True) # Keep it on top
        
        ctk.CTkLabel(ext_win, text="Filter & Export Unique Entities", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
            ...
            if filepath:
                try:
                    with open(filepath, 'w', encoding='utf-8') as f:
                        f.write(f"--- Extracted {sel} ---\n\n")
                        f.write(box.get("1.0", "end"))
                    messagebox.showinfo("Success", f"{sel} exported successfully to TXT!")
                except Exception as e:
                    messagebox.showerror("Error", str(e))

        export_btn = ctk.CTkButton(ext_win, text="Download List", command=export_list, fg_color="#2FA572", hover_color="#1D7A50", height=40)
